...
export async function verifyGitHubOidc(token: string, audience: string, now = Math.floor(Date.now() / 1000)): Promise<GitHubClaims> {
  ...
  if (!valid || claims.iss !== "https://token.actions.githubusercontent.com" || !audiences.includes(audience)
    ...
    || !claims.sub || !claims.repository || !claims.run_id || !claims.run_attempt) {
...
export async function exchangeGitHubOidc(request: Request, env: Env): Promise<Response> {
  ...
  if (claims.event_name === "pull_request") {
    if (!Number.isSafeInteger(body.pullRequestNumber) || Number(body.pullRequestNumber) <= 0) {
      throw new HttpError(400, "pull_request_required", "pullRequestNumber is required for pull request OIDC exchange");
    }
    ...
    trustClass = await classifyPullRequestFork(env, installation.installation_id, project.repository_owner, project.repository_name, Number(body.pullRequestNumber))
      ? "fork_isolated" : "first_party";
  }
  ...
  return json({ token: `sd_oidc_${await signJson(grant, env.TOKEN_PEPPER)}`, expiresAt, trustClass });
}
