...
import { parseGitHubRepository, projectSetupPath, projectSlug, setProjectFormError } from "../src/project-form.js";
...
test("manual project errors are safe even if optional markup is unavailable", () => {
  assert.doesNotThrow(() => setProjectFormError({ querySelector: () => null }, "Creation failed"));
  const element = { hidden: true, textContent: "" };
  const form = { querySelector: () => element };
  setProjectFormError(form, "Creation failed");
  assert.deepEqual(element, { hidden: false, textContent: "Creation failed" });
