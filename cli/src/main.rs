...
async fn upload(api: &Api, args: UploadArgs, json_output: bool) -> Result<()> {
    ...
    loop {
        ...
        stream::iter(page.uploads.into_iter().filter(|upload| upload.state != "published"))
    ...
    for _ in 0..120 {
        ...
        if matches!(state, "complete" | "failed" | "canceled" | "timed_out") {
            return print_value(json_output, &status);
        }
...
async fn scan(root: &Path) -> Result<Vec<ManifestEntry>> {
    ...
    for path in files {
        ...
        let (width, height) = png_dimensions(&bytes).with_context(|| format!("invalid PNG: {}", path.display()))?;
        ...
        entries.push(ManifestEntry { name: relative, sha256: hex::encode(Sha256::digest(&bytes)), byte_size: metadata.len(), width, height, path });
    }
    Ok(entries)
}
...
impl Api {
    async fn request<T: DeserializeOwned>(&self, method: Method, path: &str, body: Option<&serde_json::Value>) -> Result<T> {
        let mut request = self.client.request(method, format!("{}{path}", self.endpoint)).bearer_auth(&self.token);
        if let Some(body) = body { request = request.json(body); }
        let response = request.send().await?;
        let status = response.status();
        let bytes = response.bytes().await?;
        if !status.is_success() { bail!("SnappyDiff returned {status}: {}", String::from_utf8_lossy(&bytes)); }
        serde_json::from_slice(&bytes).context("SnappyDiff returned an invalid response")
    }
    ...
    async fn upload_file(&self, target: &UploadTarget, path: &Path) -> Result<()> {
        ...
        let response = request.send().await?;
        if !response.status().is_success() { bail!("image upload failed with {}", response.status()); }
        Ok(())
    }
}
